...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `pikvm_auto`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in pikvm_auto.__all__ or not hasattr(pikvm_auto, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in pikvm_auto.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("pikvm_auto")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "pikvm_auto" or item.name.startswith("pikvm_auto."))
        ):
